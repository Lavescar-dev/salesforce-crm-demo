<script lang="ts">
	...
	let leadId = $page.params.id;
	let existingLead = leadsStore.getById(leadId);

	if (!existingLead) {
		toastStore.error('Lead not found!');
		goto('/sales/leads');
	}
	...
	let formData = $state({
		...
		rating: existingLead?.rating || 'Warm',
		industry: existingLead?.industry || '',
		annualRevenue: existingLead?.annualRevenue || 0,
		numberOfEmployees: existingLead?.numberOfEmployees || 0,
		description: existingLead?.description || '',
		street: existingLead?.address?.street || '',
		city: existingLead?.address?.city || '',
		state: existingLead?.address?.state || '',
		postalCode: existingLead?.address?.postalCode || '',
		country: existingLead?.address?.country || 'USA'
	});
	...
	function handleSubmit(e: Event) {
		...
		if (!leadId) {
			toastStore.error('Lead ID is missing.');
			return;
		}

		try {
			const updatedLead = leadsStore.update(leadId, {
			...
			if (updatedLead) {
				toastStore.success('Lead updated successfully!');
				goto(`/sales/leads/${updatedLead.id}`);
			} else {
				toastStore.error('Failed to update lead: Lead not found.');
			}
		} catch (error) {
			console.error('Error updating lead:', error);
			toastStore.error('Failed to update lead.');
		}
	}
</script>
...
<div class="max-w-4xl mx-auto">
	<div class="mb-6">
		<h1 class="text-3xl font-bold text-salesforce-gray-900">Edit Lead</h1>
		<p class="text-salesforce-gray-600 mt-1">Update lead information</p>
	</div>
	...
	<form onsubmit={handleSubmit}>
		...
		<div class="mt-6 flex gap-3 justify-end">
			<Button type="button" variant="secondary" onclick={() => goto(`/sales/leads/${leadId}`)}>
				Cancel
			</Button>
